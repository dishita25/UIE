...
def get_config():
    ...
    parser.add_argument("--niter", type=int, default=51, help="Number of iterations")
...
#                         print(f"Original batch shapes - X: {X_batch.shape}, Y: {Y_batch.shape}")
#                         print(f"Current scale {scale_num} shapes - X: {X.shape}, Y: {Y.shape}")
#                         print(f"Expected scale {scale_num} size: {HARDCODED_SCALES[scale_num]}")
    ...
    try:
        ...
        for scale_num in range(len(HARDCODED_SCALES)):
            ...
            for epoch in range(opt.niter):
                ...
                if val_loader:
                    val_ssim, val_psnr = validate_cyclegan_model(G, val_loader, opt.device, scale_num)
                    print(f"Scale {scale_num}, Epoch {epoch} - Val SSIM: {val_ssim:.4f}, PSNR: {val_psnr:.4f}")
                    ...
                    if not opt.disable_wandb:
                        wandb.log({
                            ...
                            "val_epoch": epoch,
                            "val_scale": scale_num,
                            "val_total_epoch": scale_num * opt.niter + epoch,
                        })
...
def save_basic_cyclegan_samples(G, F, X_pyramid, Y_pyramid, opt, scale_num, epoch):
    """Save basic CycleGAN samples and log to WandB"""
    with torch.no_grad():
        X_curr = X_pyramid[scale_num][:4]  # First 4 blur images
        Y_curr = Y_pyramid[scale_num][:4]  # First 4 real images
        ...
        # Generate translations
        fake_Y = G(X_curr)  # X -> Y (blur -> real)
        fake_X = F(Y_curr)  # Y -> X (real -> blur)
        
        print(f"Shape of fake_Y: {fake_Y.shape}")
        
        # Create comparison: [Blur | Generated_Y | Real | Generated_X]
        comparison = torch.cat([X_curr, fake_Y, Y_curr, fake_X], dim=0)
