...
# New utility to align tensor shapes by center-cropping
def align_tensors(ref, *tensors):
    """
    Crop (centered) all input tensors to exactly match the shape of the reference tensor.
    Returns a tuple (t1_aligned, t2_aligned, ...)
    """
    ...
    for t in tensors:
        ...
        y0 = max(0, (h - H) // 2)
        x0 = max(0, (w - W) // 2)
        results.append(t[..., y0:y0+H, x0:x0+W])
...
def train_single_image_with_funiegan(opt):
    ...
    for scale_num in range(opt.stop_scale + 1):
        ...
        for epoch in range(opt.niter):
            ...
            # Align blur_img, prev, and noise_ to real_img at this point before discriminator/generator input
            blur_img, prev, noise_ = align_tensors(real_img, blur_img, prev, noise_)
            ...
            # Padding noise input to multiple of 16 if needed
            h, w = noise.shape[2], noise.shape[3]
            pad_h = (16 - h % 16) % 16
            pad_w = (16 - w % 16) % 16
            ...
            # Align fake and real_img for loss calculation
            fake_aligned, real_aligned = align_tensors(real_img, fake, real_img)

            loss_adv = mse(fake_pred, torch.ones_like(fake_pred))

            loss_l1 = l1(fake_aligned, real_aligned)
            loss_vgg = perceptual(fake_aligned, real_aligned)
